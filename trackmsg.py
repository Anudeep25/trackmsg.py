#!/usr/bin/python
# Added date and time to the script
import argparse
import re
...
id_regex = re.compile(" [A-F0-9]{7,11}")

date_regex = re.compile("^.{0,6}")
time_regex = re.compile("^.{7}(.{8})")
...
table = PrettyTable(["Message ID", "Date", "Time" ,"Status", "From", "To", "Score", "Relay Server", "Sender Server", "Extended Status"])
        ...
        if phrase.match(line):
            ...
            if message_ids:
                ...
                if not message_id_regex in search_tokens:
                    if args.server or (args.sender and not client_regex.match(line)) or (
                            ...
                            not from_regex.match(line) and not client_regex.match(line))):
            ...
            if message_id:
                if not message_id in messages_dict:
                    ...
                    messages_dict[message_id]['time'] = None
                ...
                message_id_date = date_regex.findall(line)
                if message_id_date:
                    messages_dict[message_id]['date'] = wrap_always(message_id_date[0], 7)


                message_id_time = time_regex.findall(line)
                if message_id_time:
                    messages_dict[message_id]['time'] = wrap_always(message_id_time[0], 10)
                ...
                if status:
                    ...
                    table.add_row([messages_dict[message_id]['id'], messages_dict[message_id]['date'], messages_dict[message_id]['time'], messages_dict[message_id]['status'],
                                   messages_dict[message_id]['sender'], messages_dict[message_id]['recipient'],
                                   messages_dict[message_id]['score'], messages_dict[message_id]['relay'],
                                   messages_dict[message_id]['server'], messages_dict[message_id]['status_extended']])
                    break
